...
| G3 | 素材页（首页素材库）保留顶部标签选项 | ⬜ | 现有标签筛选保留 |
| G4 | 素材页图片固定比例 16:9，每行 4 张图，每张为一个素材合集 | ⬜ | 点击合集后弹窗展示（见图 1） |
| G5 | 素材合集弹窗：左侧大图预览，右侧从上到下依次为 合集标题、作者、素材列表、合集说明、下载按钮 | ⬜ | 下载按钮：打包下载该合集下所有图片 |
...
| A3 | 点击封面后不使用弹窗，改为平铺、固定宽度、瀑布流布局 | ⬜ | 保留灯箱查看效果 |
| A4 | 保留灯箱查看效果 | ⬜ | 与平铺布局结合 |
...
| B3 | 导航栏播放器展开播放列表交互逻辑不明显，用户不知点击哪里展开 | 展开入口不清晰 | `nav-player.js` 播放器 | 🐛 待修复（大小屏均需支持） |
| B4 | 播放列表宽度应与播放器长度一致 | 列表宽度未跟随播放器 | `nav-player.js` 播放列表 | 🐛 待修复（大小屏均需支持） |
| B5 | 小屏模式下播放器未显示当前歌手与歌名 | 小屏布局缺失歌曲信息 | `nav-player.js` 小屏布局 | 🐛 待修复 |
...
| 19.2 | 素材页保留标签 + 16:9 每行 4 张合集卡片 | 中 | G3 / G4 | ⬜ ① `data.js` 的 `galleryData` 重组为**合集结构**（`title`/`author`/`cover`/`images[]`/`desc`）② 顶部分类筛选 Tab **保留**，筛选维度按"合集" ③ 卡片 16:9，大屏 4 列 / 平板 2 列 / 手机 1 列（`style.css` + `gallery.js` `initGallery`） |
| 19.3 | 素材合集弹窗（左大图预览 / 右标题·作者·列表·说明·下载） | 中 | G5 | ⬜ ① `gallery.js` 点击卡片弹窗：左侧大图预览，右侧从上到下为 合集标题 / 作者 / 素材列表（合集内**所有图片缩略图**，点击切换左侧大图）/ 合集说明 / 下载按钮 ② 引入 **JSZip**（`assets/js/`），下载按钮打包下载该合集全部图片 ③ `style.css` 弹窗布局 |
| 19.4 | 相册页封面点击改为平铺瀑布流（保留灯箱） | 中 | A3 / A4 | ⬜ ① `album.js` `initAlbum`：去掉弹窗，点击封面**当前页内展开**瀑布流（CSS `columns` 容器定宽、固定列数），支持**返回列表且不刷新页面** ② 点击瀑布流图片仍开**灯箱**看大图（`style.css`） |
...
| 23 | 横竖屏标识自动化 | 难 | D2 | ⏸ 暂缓 |
| 24 | 导航栏播放器：展开列表交互优化 + 列表宽度与播放器等宽 + 小屏显示歌手歌名 | 难 | B3 / B4 / B5 | 🐛 待修复（大小屏均需支持） |
