...
> 顶部导航播放器（G5 / `NavPlayer.vue`）已于 2026-08-07 完成交互与双端样式实现并通过验收；与底部大播放器互通（T16 范围）仍暂缓。
...
| 🟡 常规·易 | 页面等价、并行推进 | T10 素材库、T11 相册、T12 个人资料、T13 成长 |
| 🟡 常规·难 | 复杂交互、媒体互斥 | T15 视频页、T16 音视频互斥 |
| ⚪ 暂定 | 本轮不做、待定范围 | T17 留言与建议弹幕（合并·暂定） |
...
### 🟡 T16 · 双播放器互斥（FE，中，常规）★ 音频/视频互斥
- **状态**：✅ 已进入实现（2026-08-07 需求变更）。原"双播放器互通"方案调整——**仅保留顶部导航音频播放器，取消底部大播放器**；改为音频与视频**互斥**：播放音频时暂停视频，播放视频时暂停音乐，同一时刻只播一种媒体。
- **实现**：`useDataStore` 持有单一 `<audio>` 实例 + `activeMedia('none'|'audio'|'video')` 互斥标记；音频 `play()` 前 `pauseVideo()`，视频 `playVideo()/togglePlay()` 前 `pauseAudio()`；VideoView 注册 `registerVideoPauser` 供音频暂停视频。
- **验收**：音乐播放中切到视频页播放 → 音乐暂停；视频播放中点导航播放器播放 → 视频暂停；进度/音量/循环仍由 store 统一。
- **关联原则**：P1 状态收敛、原则 4 双播放器特征（互斥版）。

### ⚪ T17 · 留言与建议弹幕（合并·暂定需求）
> **需求变更（2026-08-07）**：原「T14 留言册」与「T17 建议弹幕页（G1）」**合并为一项**，并标记为**暂定需求**，本轮不做实现。
- **合并范围**：留言页（原 T14 `/message`）+ 建议弹幕页（原 T17 B站弹幕风格右→左滚动 + 点赞数）合并为「留言与建议」模块，统一入口与设计。
- **状态**：⚪ **暂定（本轮不做）**，待范围明确后拆分 Task。
- **关联原则**：原则 2/4。
...
### F. 留言与建议（合并·暂定需求）
> **需求变更（2026-08-07）**：原「留言页（T14）」与「建议弹幕页（T17 / G1）」合并为一项，标记为**暂定需求**，本轮不实现。
- ⚪ F1 留言列表渲染（XSS 安全，禁 v-html）— 暂定
- ⚪ F2 建议弹幕：B站弹幕风格右→左滚动 + 点赞数 — 暂定
- ⚪ F3 合并后统一入口与设计，待范围明确
...
- ✅ G5 顶部常驻音频播放器（`NavPlayer.vue` 已挂载实现交互，大屏/小屏样式已对齐验收；**底部大播放器已取消**，音视频互斥见 T16）

**G5 详细界面效果（依据图片备注 + `pages.md` + `nav-player.js` + `style.css`）**：

1. **整体触发方式与入口位置（图片 1/2/3 备注）**
   - 播放器入口位于顶部导航栏右侧，为圆形头像按钮。**收起态**：圆形按钮背景为**白色半透明渐变**（原紫色渐变已改为白色半透明渐变，含轻微毛玻璃质感），中央为音符图标，图标静态。**展开态**：显示当前歌曲封面头像，白色渐变背景隐藏。
...
2. **左侧控制条（`nav-player-left-section`）**
   - 宽度 110px（大屏/小屏/≤480px 统一 110px，确保 4 个控制按钮完整不被头像遮挡），从头像左侧弹性展开。
...
4. **统一背景面板（`nav-player-zone.expanded::after`）**
   ...
   - 背景色 `rgba(255,255,255,0.15)`，边框 `rgba(255,255,255,0.08)`，圆角 **7px（原 14/10px 减半）**，`backdrop-filter: blur(20px) saturate(160%)`，阴影 `0 4px 20px rgba(0,0,0,0.15)`。
   - 背景有从 scaleX(0.6) 弹开到 scaleX(1) 的入场动画。
   - 列表打开时，面板与列表衔接处取消对应圆角，形成一体（列表顶两角 + 面板底两角取消，不重叠，顶部留 8px 间隙）。

5. **播放列表弹层（`nav-player-playlist`）**
   - 位于面板下方，左对齐面板左边缘，顶部间距 **8px**（避免与播放器重叠），大屏宽度 **258px**（110+4+40+8+96，与面板右边缘对齐），小屏宽度 `calc(110px+4px+36px+6px)`（含头像右侧留白，与播放器右侧对齐）。
...
6. **交互细节（`nav-player.js` 行为）**
   - 点击收起态播放器头像按钮：图标先执行**向右旋转 180° 的过渡动画**（约 0.3s ease），旋转完成后整体面板平滑展开为完整播放器（左侧控制条 + 头像 + 右侧信息条依次铺开）；展开时头像从 0.8 缩放到 1，白色半透明渐变背景隐藏，露出封面头像。
   - 点击列表按钮：切换播放列表显隐；展开面板时若列表已打开则关闭，反之亦然。
   - 播放/暂停：按钮图标在「播放 ▶」与「暂停 ‖」之间切换；未播放时头像内音符图标静止，播放时音符图标持续旋转。
   - 上一曲/下一曲：切歌后自动更新头像、右侧歌名、列表高亮。
   - 点击列表项：切换到对应歌曲并自动播放。
   - 展开态下再次点击头像：面板收起，控制条与信息条向中间收缩，头像恢复为白色半透明渐变 + 音符图标（收起动画可与展开反向，图标回正）。

7. **移动端适配（≤768px）**
   - 头像按钮尺寸缩小为 36×36px。
   - **整体定位**：播放器整体位于汉堡导航栏图标的**左侧**（汉堡菜单在导航最右端，播放器紧贴其左侧），而非独立居右。
   - 右侧面板隐藏，仅左侧控制条展开。
   - 统一背景只覆盖左侧面板 + 头像按钮。
...
| 角色 | 主负责 Task | 协同 |
|------|-------------|------|
| **前端 A**（用户端） | T01(FE)、T06、T08、T10、T11、T12、T13、T15、T16 | T02、T04、T18、T19、T20（T17 暂定） |
...
| **M3 页面等价** | T08、T10~T16 | 6 页功能等价 + 双端验收 + G/A 待办落地（T17 合并暂定） |
