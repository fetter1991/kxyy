...
- **当前占位**：顶部导航播放器 `NavPlayer.vue` 已于 2026-08-07 挂载实现（展开/收起、图标右旋 180°、播放列表、真实 `<audio>` 播放、5 秒空闲/点击外部自动收起、双端样式对齐验收）；与底部大播放器互通状态（下方第 8 节）仍暂缓，待 T16 落地时接入共用 store。
- **目标**：显式状态管理替代隐式契约互斥。
...
- ✅ G5 双播放器常驻导航（`NavPlayer.vue` 已挂载实现交互，大屏/小屏样式已对齐验收；与底部大播放器互通见 T16 暂缓）
