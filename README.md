...
| 顺序 | 任务 | 难度 | 对应条目 | 改动说明 |
|------|------|------|----------|----------|
| 20 | 视频播放器 Tab 专辑切换 | 难 | V1 | 新增 Tab 组件 + 按专辑分组数据 + 切换时重渲染播放列表 |
| 21 | 新增「建议」弹幕页 | 难 | G1 | 新建 `pages/suggest.html` + 弹幕动画（右→左滚动、点赞数展示）组件 |
| 22 | videoData 按文件夹自动生成 | 难 | D1 | 需引入构建脚本或前端读取目录，替代手动维护 |
| 23 | 横竖屏标识自动化 | 难 | D2 | 通过文件名约定或读取视频 metadata 自动判断，去除手动字段 |
